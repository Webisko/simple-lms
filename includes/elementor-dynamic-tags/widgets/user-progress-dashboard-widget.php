...
class User_Progress_Dashboard_Widget extends Widget_Base {
    ...
    protected function render(): void {
        ...
        if (empty($courses)) { echo '<div class="elementor-alert elementor-alert-info">'.esc_html__('Brak kursów na koncie.','simple-lms').'</div>'; return; }
        ...
        foreach ($courses as $course) {
            ...
            echo '<div class="card-meta" style="font-size:0.9em; opacity:0.85">'.esc_html(sprintf(__('Ukończone lekcje: %d/%d (%d%%)','simple-lms'), $completed_lessons, $total_lessons, (int)$progress)).'</div>';
            ...
            echo '<a class="btn-continue" href="'.esc_url($continue_url).'" style="display:inline-block; padding:10px 16px; background:#2196F3; color:#fff; text-decoration:none; border-radius:4px">'.esc_html__('Kontynuuj','simple-lms').'</a>';
            echo '</div></div></div>';
        }
        echo '</div>';
